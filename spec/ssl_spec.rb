...
describe 'percona::ssl' do
  platform 'ubuntu'
  override_attributes['percona']['encrypted_data_bag'] = 'test-bag'
  override_attributes['percona']['server']['role'] = %w(master slave)

  before do
    stub_data_bag_item('test-bag', 'ssl_replication').and_return(
